...
const Client = ({id}) => {
  ...
  const refreshData = async () => {
    ...
    setProjects(data.filter((project)=> project.client.toString() === id ));
  };
      ...
      <div className="max-w-11/12 mx-auto ">
          ...
          <div className="grid grid-cols-3 gap-2 max-sm:grid-cols-1 max-md:grid-cols-2 ">
            {
              ...
              filteredProjects.map((project)=>   <ProjectCards key={project._id} project={project} role="client"  /> )
            }
          </div>
        </div>
        <hr />
        <div className="my-5">
          ...
          <div className="grid grid-cols-3 gap-2 max-sm:grid-cols-1 max-md:grid-cols-2 ">
            ...
            {
              completedProjects.map((project)=>   <ProjectCards key={project._id} project={project} role="client" /> )
            }
          </div>
        </div>
      </div>
